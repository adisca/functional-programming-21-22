...
compare (Interval intA) (Interval intB) =
    let
        maybeDateCompare : Maybe Date -> Maybe Date -> Order
        maybeDateCompare md1 md2 =
            case md1 of
                Just d1 ->
                    case md2 of
                        Just d2 -> Date.compare d1 d2
                        Nothing -> LT
                Nothing ->
                    case md2 of
                        Just _ -> GT
                        Nothing -> EQ
    in
    Date.compare intA.start intB.start |> chainCompare (maybeDateCompare intA.end intB.end)
...
view interval =
    case interval of
        Interval int ->
            div [class "interval"]
            [
                int.start   |> Date.dateToString >> text >> List.singleton
                            |> div [class "interval-start"]

                , case int.end of
                    Just d ->
                        d   |> Date.dateToString >> text >> List.singleton
                            |> div [class "interval-end"]

                    Nothing ->
                        "Present"   |> text >> List.singleton
                                    |> div [class "interval-end"]

                , case interval |> length of
                    Just (years, months) ->
                        "(" ++ (years |> String.fromInt) ++ ", " ++ (months |> String.fromInt) ++ ")"
                            |> text >> List.singleton
