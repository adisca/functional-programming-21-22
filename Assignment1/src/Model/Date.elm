module Model.Date exposing (Date, Month(..), compare, compareMonth, full, month, monthToString, monthsBetween,
    monthsBetweenMonths, offsetMonths, onlyYear, view, year, dateToString)
...
monthsBetween dA dB =
    case dA |> month of
        Just month1 ->
            case dB |> month of
                Just month2 ->
                    (((year dA) - (year dB)) * 12 + (monthToInt month1) - (monthToInt month2))  |> abs >> Just
                Nothing -> Nothing
        Nothing ->
            case dB |> month of
                Just _ -> Nothing
                Nothing -> ((year dA) - (year dB)) * 12 |> abs >> Just
...
compare (Date d1) (Date d2) =
    let
        monthCompare : Maybe Month -> Maybe Month -> Order
        monthCompare m1 m2 =
            case m1 of
                Just month1 ->
                    case m2 of
                        Just month2 -> Basics.compare (monthToInt month1) (monthToInt month2)
                        Nothing -> LT
                Nothing ->
                    case m2 of
                        Just _ -> GT
                        Nothing -> EQ
    in
    Basics.compare d1.year d2.year  |> chainCompare (monthCompare d1.month d2.month)
...
{-| A utility function I implemented to be reused in other modules
...
dateToString d =
    case d |> month of
        Just m ->
                (monthToString m ++ " " ++ (d |> year >> String.fromInt))
        Nothing -> d    |> year >> String.fromInt


view : Date -> Html msg
view (Date d) =
    d   |> Date >> dateToString -- re-wrapping d to Date so I can keep dateToString more general
        |> text >> List.singleton
        |> div [class "date"]
...
monthsBetweenMonths m1 m2 =
    monthToInt m1 - monthToInt m2 |> abs
