...
atLeastOneIngredient = test "each recipe contains at least one ingredient" <|
  \_ -> 
...
atLeastOneIngredientClass = test "each recipe contains at least one ingredient (using classes)" <|
    \_ -> 
      RecipeView.recipeView RecipeView.brownies
        |> Q.fromHtml
...
eachIngredientHasClassIngredient = test "each ingredient has class ingredient" <|
  \_ -> 
    RecipeView.recipeView RecipeView.brownies
      |> Q.fromHtml
      |> Q.find [S.tag "ul"]
